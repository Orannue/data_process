...
def merge_short_segments(segments, min_segment_len_frames):
    ...
    for start, end in segments[1:]:
        ...
        # 当前段太短，直接并入下一段。
        if current_len < min_segment_len_frames:
            merged[-1][1] = end
            continue

        # 下一段太短，也并回当前段，避免碎片化。
        if next_len < min_segment_len_frames:
            merged[-1][1] = end
        else:
    ...
    if len(merged) > 1:
        ...
        if last_len < min_segment_len_frames:
...
def process_movie_scenes(json_data, moviebench_path, output_base_path):
    ...
    for movie_rank, (movie_id, scenes) in enumerate(sorted_movies, start=1):
        ...
        for scene_idx, (scene_desc, scene_videos) in enumerate(sorted_scenes):
            ...
            clip_level_segments = []
            global_offset = 0
            min_scene_len_frames = max(12, int(round(fps_for_detection * 0.8)))
            ...
            # --- 步骤 2: 合并过短段，减少误切分 ---
            min_shot_len_frames = max(15, int(round(fps_for_detection * 1.2)))
            shot_segments = merge_short_segments(clip_level_segments, min_shot_len_frames)
